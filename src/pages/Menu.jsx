            ...
            <div className="w-full overflow-x-auto no-scrollbar pb-2 pl-4 px-4 py-4 pt-6">
                <div className="flex space-x-3 min-w-max pr-4">
                    <button
                        ...
                        className={`flex items-center justify-center px-5 py-2 rounded-full text-sm font-medium font-playfair transition-transform active:scale-95 animate-fade-up ${activeCategory === "Todo" ? "bg-primary text-primary-content shadow-md shadow-primary/20" : "bg-white dark:bg-white/10 border border-border-subtle dark:border-white/10 text-text-main dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5"}`}
                    >
                        Todo
                    </button>
                    {categories.map((cat, i) => (
                        <button
                            ...
                            className={`flex items-center justify-center px-5 py-2 rounded-full text-sm font-medium font-playfair transition-colors animate-fade-up ${activeCategory === cat.id ? "bg-primary text-primary-content shadow-md shadow-primary/20" : "bg-white dark:bg-white/10 border border-border-subtle dark:border-white/10 text-text-main dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5"}`}
                    ...
                    <section key={category.id} className="bg-background-card dark:bg-white/5 rounded-2xl border border-border-subtle dark:border-white/10 overflow-hidden shadow-sm animate-fade-up opacity-0" style={{ animationDelay: `${index * 100}ms`, animationFillMode: 'forwards' }}>
                        <div className="relative h-32 w-full overflow-hidden">
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-4">
                                <h2 className="text-white text-xl font-bold tracking-wide animate-fade-up delay-100 font-hairline font-playfair">{category.title}</h2>
                            </div>
                        </div>
                            ...
                            {category.items.map((item, i) => (
                                <div key={i} className="p-4 md:p-4 md:rounded-xl md:bg-gray-50 md:dark:bg-white/5 flex items-center justify-between group cursor-pointer hover:bg-gray-50 dark:hover:bg-white/5 md:hover:bg-gray-100 md:dark:hover:bg-white/10 transition-colors animate-fade-up opacity-0" style={{ animationDelay: `${(i * 50) + 200}ms`, animationFillMode: 'forwards' }}>
                                    <div className="flex flex-col gap-1 pr-4">
                                        <h3 className="font-semibold text-text-main dark:text-gray-100 text-base font-bold font-playfair">{item.name}</h3>
                                        <p className="text-xs text-secondary dark:text-gray-400 line-clamp-2 font-normal font-playfair italic">{item.description}</p>
                                    </div>
